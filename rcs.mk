...
define run-worktree-add
	...
	git -C $(SRCDIR)/$(1) worktree add -b $(3) worktree/$(3) $(2); \
	if [ $$? -eq 0 ]; then \
...
define run-worktree-remove
	echo -e "\n---- removing worktree/$(2)"; \
	git -C $(SRCDIR)/$(1) worktree remove worktree/$(2) 2>/dev/null; \
...
rcsmaster_liburcu=origin/master
rcsmaster_lttngust=origin/master
rcsmaster_lttngtools=origin/master
...
stable-2.10.add stable-2.12.add stable-2.13.add rcs10.add rcs12.add rcs13.add rcsmaster.add:
	$(TRACE)
	$(Q)$(foreach repo, $(REPOS),$(call run-worktree-add,$(repo),$($(branch)_$(repo)),$(branch)); )
